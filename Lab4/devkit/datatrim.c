/*
The script to fetch the subset of the original data and add links to the node with no out going
links The index is remapped to make it continuous in the resulting subset of data. Index start from
0.

As for the output file, first line is the number of the included nodes, the following lines
indicating the directed links with the first number as the index of the source node and the second
number as the index of the destination node.
...
int main(int argc, char *argv[]) {
    ...
    if (b_extend) {
        ...
        printf("There are %d nodes and %d edges in the sub dataset including the added edges for "
               "the nodes with no outgoing links. \n",
               Ncount, Ecount);
    } else
